...
public class Block implements Collidable, Sprite, HitNotifier {
    ...
    public Velocity hit(Ball hitter, Point collisionPoint, Velocity currentVelocity) {
        ...
        //decreases the amount of hits left
        if (this.hitPoints.getValue() > 0) {
            this.hitPoints.decrease(1);
        }
        /*if the collision point is on the upper side or on the downer side of the block */
        if (collisionPoint.getY() == rect.getDownerLine().start().getY()
                || collisionPoint.getY() == rect.getUpperLine().start().getY()) {
            // change the 'y' value of the velocity.
            currentVelocity = new Velocity(currentVelocity.getDx(), currentVelocity.getDy() * -1);
        }
        /*if the collision point is on the left side or the right side of the block.*/
    ...
    public void drawOn(DrawSurface d) {
        ...
        if (this.image.size() > 0) {
            d.drawImage((int) rect.getUpperLeft().getX(), (int) rect.getUpperLeft().getY(),
                    this.image.get(this.image.size() - this.hitPoints.getValue()));
        } else {
    ...
    /**
     * a boolean method that returns true if the hit counter is 0.
     *
     * @return true if the hitcounter has reached 0, false otherwise.
     */
    public boolean hasReachedZeroHits() {
        return this.getHitPoints() == 1;
    }

    /**
     * sets a random color for the block.
     */
    public void setRandomColor() {
        Random rand = new Random();
        this.setFillColor(new Color(rand.nextInt(250), rand.nextInt(250), rand.nextInt(250)));
    }
}
